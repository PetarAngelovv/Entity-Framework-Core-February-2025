...
namespace CarDealer
{
    public class StartUp
    {
        ...
        {
            var customers = context.Customers
                   .Where(c => c.Sales.Any())  // Уверяваме се, че има поне една продажба за клиента
                   .Select(c => new
                   {
                       fullName = c.Name,
                       boughtCars = c.Sales.Count,
                       spentMoney = c.Sales
                                       .SelectMany(s => s.Car.PartsCars) // Извличаме всички части за всяка кола
                                       .Sum(pc => pc.Part.Price) // Сумираме цените на частите
                   })
                   .OrderByDescending(c => c.spentMoney)  // Първо по похарчена сума в низходящ ред
                   .ThenByDescending(c => c.boughtCars)  // След това по броя на купените коли
                   .ToList();  // Променяме на ToList(), за да можем да дебъгваме резултата

            // Добавяме проверка дали списъкът не е празен
            if (customers.Count == 0)
            {
                return "No customers found with at least one car purchased.";
            }
            ...
            //  then by total bought cars again in descending order. Export the list of customers to JSON in the format provided below.
