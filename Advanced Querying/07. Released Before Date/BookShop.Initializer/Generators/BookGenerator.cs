    ...
    class BookGenerator
    {
        ...
        internal static Book[] CreateBooks()
        {
            ...
            for (int i = 0; i < bookCount; i++)
            {
                ...
                decimal price;
                if (!decimal.TryParse(bookTokens[3], NumberStyles.Any, CultureInfo.InvariantCulture, out price))
                {
                    Console.WriteLine($"Invalid price format: {bookTokens[3]}"); // Дебъгване
                    price = 0; // Или друга стойност по подразбиране
                }
